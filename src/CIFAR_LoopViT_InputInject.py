...
class CIFARLoopViTInputInject(nn.Module):
    """Looped ViT with input injection for CIFAR.
    
    Core innovation: At each loop iteration, the initial hidden state (patch embeddings
    + positional + CLS) is added back as a residual, i.e.:
        h_t = Layer(h_{t-1}) + h_0
    
    This provides direct gradient path to early layers and preserves information
    ...
    def forward(
        ...
        # Prepend CLS token if used (before positional)
        ...
        # Preserve initial state for input injection
        initial_hidden = x
        running_hidden = x
        ...
        for step in range(self.max_loop_steps):
            # Input injection: add initial hidden state
            running_hidden = running_hidden + initial_hidden
